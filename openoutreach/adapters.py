...
"""Adapters for 9router LLM gateway, Free Real Lead Discovery, and WhatsApp Integration.
...
3. Free Real Lead Discovery & WhatsApp Integration (discovers real B2B leads using DuckDuckGo web search
   and qualifies them with pt-BR consultative reasoning).
"""
...
# ── 3. Main Hook Installation ──────────────────────────────────────────────────

def install_adapters() -> None:
    """Install all hooks for 9router and Free Real Lead Discovery."""
    global _INSTALLED
    if _INSTALLED:
        return
    _INSTALLED = True
    ...
    # 2. Free Lead Discovery & Readiness
    try:
        ...
        def custom_missing():
            from openoutfind.core.config import SiteConfig
            res = orig_missing()
            # If BetterContact key is not configured, the free web discovery provider is active
            if "bettercontact" in res and not SiteConfig.load().bettercontact_api_key:
                del res["bettercontact"]
            # Email finder is intentionally not required (100% WhatsApp platform)
            if "email_finder" in res:
                del res["email_finder"]
            return res
    ...
    try:
        ...
        provider_mod.active = lambda: None
    except Exception as exc:
    ...
    try:
        ...
        ready_pool_mod.promote_to_ready = lambda qualifier: 0
    except Exception as exc:
